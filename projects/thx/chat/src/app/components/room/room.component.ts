...
export class RoomComponent implements OnInit, OnDestroy {
  ...
  localNotifications: boolean = true;

  userHasPush: boolean = false;
  ...
  ngOnInit(): void {
    ...
    const userHasPushSub: Subscription = this.userHasPushNotifications().subscribe({
      next: (hasPush: boolean) => {
        this.userHasPush = hasPush;
        userHasPushSub.unsubscribe();
      },
      error: (e: any) => {
        console.error(e);
        userHasPushSub.unsubscribe();
          ...
          if (!this.chatService.isRoomSubscribed(roomId)) {
            ...
            this.chatService.subscribeRoom(roomId).subscribe({
              next: (room: Room | null) => {
                ...
                if (room) {
                  this.room = room;                  
                  this.localNotifications = this.chatService.isRoomSubscribed(room.id);
                  console.log('localNotifications', this.chatService.isRoomSubscribed(room.id));
    ...
    this.onMessageSub = this.chatService.onMessage.subscribe({
      next: (result: RoomMessage) => {
        ...
        if (this.room && result.roomId === this.room.id) { // show only relevant messages for room (exclude localNotifications from other rooms)
  ...
  openSettings(): void {
    this.localNotifications = true; // set localNotifications true temporary 
  ...
  leaveRoom(): void {
    ...
    if (!this.localNotifications) {
      const dialogSub: Subscription = this.dialogService.openDialog({
        ...
        content: $localize `Are you going to leave the chat permanently? If you want to receive localNotifications, turn localNotifications for this chat on before leaving.`,
        actions: [
  ...
  unsubscribeRoom(): void{
    if (this.room) {
      ...
      const dialogSub: Subscription = this.dialogService.openDialog({
        title: $localize `Leave chat?`,
        content: $localize `You are about to leave the chat permanently. If you want to stay subscribed and receive localNotifications, use the arrow on the left instead.`,
        actions: [
          { title: $localize `Leave`, value: 'leave' }
        ]
      }).subscribe({
  ...
  toggleLocalNotifications(): void {
    this.localNotifications = !this.localNotifications;
    if (this.room)
    if (this.localNotifications) {
  ...
  userHasPushNotifications(): Subject<boolean> {
    // console.log('userHasPush?');
    return this.chatService.hasPush();
  }

  pushNotificationsAvailable(): boolean {
    return this.chatService.pushNotificationsAvailable();
  }
