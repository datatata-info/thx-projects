...
export class SigmqttService {
  ...
  connect() {
    ...
    this.toSignalMessagesSubject.subscribe((msg: any) => {
      if (this.clientsState && msg === 'new_password') {
        this.sendJoinParamsAck(this.clientsState);
      }
    });
  }
  ...
  handleWantJoin(msg: any): void {
    ...
    const state: SignalCreatorClientsState = {
      ...
      timestamp: [],
      verifier: [msg.params.verifier],
      pubKey: forge.pki.publicKeyFromPem(msg.params.pubKey),
      retries: 0,
      failed: false,
    };
    ...
    const data = {
      ...
      creatorId: this.getClientId(),
      timestamp: this.creatorStartTimer,
    };
  ...
  handleJoinParamsAck(msg: any): void {
    ...
    const decryptedData = JSON.parse(this.mqttService.keyPair['privateKey'].decrypt(atob(msg.params.data), 'RSA-OAEP'));
    ...
    if (!this.mqttService.verifyValidator(
      clientState.pubKey,
      clientState.verifier[0],
      `${clientState.clientId}|${decryptedData.timestamp}`
    )) {
      // TODO: failed verification, do something
      return;
    }
    else {
      if (clientState.timestamp.length === 0) {
        clientState.timestamp.push(decryptedData.timestamp);
      }
    }
  ...
  handleJoinParams(msg: any): void {
    ...
    if (this.clientsState) {
      ...
      const tmpPubkey = forge.pki.publicKeyFromPem(msg.params.pubKey);
      if (!this.mqttService.verifyValidator(tmpPubkey, msg.params.verifier, msg.params.data)) {
        return;
      }

      if (!this.mqttService.verifyValidator(
        tmpPubkey,
        this.clientsState.verifier[0],
        `${decryptedData.creatorId}|${decryptedData.timestamp}`
      )) {
        return;
      }

      this.clientsState.creatorId = decryptedData.creatorId;
      this.clientsState.pubKey = tmpPubkey;
      this.clientsState.state = 'await_join_ack';
  ...
  sendJoinParamsAck(clientsState: SignalClientState): void {
    const data = {
      timestamp: this.clientStartTimer,
      password: this.password
    };

    const params = {
      clientId: this.getClientId(),
      data: btoa(clientsState.pubKey.encrypt(JSON.stringify(data), 'RSA-OAEP')),
      verifier: ''
    };
    params.verifier = this.mqttService.createValidator(params.data);

    if (this.requirePassword) {
      if (clientsState.retries < MAX_PASSWORD_RETRIES) {
  ...
  handleJoinAck(msg: any, clientsState: SignalClientState): void {
    ...
    clientsState.state = 'successfully_joined';
  ...
  fsm(msg: any): void {
    ...
    if (msg !== undefined) {
      ...
      // client connecting to app part
        ...
        if (this.clientsState && msg.params.clientId === this.getClientId()) {
          if (msg.command === 'join_params') {
            this.handleJoinParams(msg);
          }
          if (msg.command === 'join_ack') {
